...
- GitHub CLI がインストールされているか確認: `gh --version`
- ログイン操作の確認: `gh auth status`
- 権限不足の場合: `gh auth refresh -s repo` で権限を更新して再試行

### PR が見つからない / アクセスエラー

- PR 番号が正しいか確認
- リポジトリの権限を確認し、正しいコンテキストで実行しているか確認

## Reference Files

詳細な操作ガイドや分類ポリシーについては、以下を参照してください:

- `references/github-cli-guide.md`: GitHub CLI の活用方法
- `references/review-classification-policy.md`: レビュー指摘の分類基準と対応方針の詳細
