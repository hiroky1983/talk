...
## [ADR-005] テストモードの導通確認メッセージの実装

- **日付**: 2025-12-30
- **決定**: Python AI Service の `PLAN_TEST` において、音声だけでなくテキストメッセージ (`text_message`) を返すように実装。
- **理由**: クライアントから見て、gRPC 連携および WebSocket 通信が正常に完了したことを視覚的に確認しやすくするため。
