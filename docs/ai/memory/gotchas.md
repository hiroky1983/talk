...
## エージェント（Antigravity）の挙動

- **gitignore による制限**: `.env` などのファイルが `.gitignore` に含まれている場合、エージェントはデフォルトで `view_file` 等のアクセスをブロックされる。これが必要な場合は、ユーザーに明示的に許可を求めるか、他の手段を検討する必要がある。
